"""
한경대학교 글로컬융합교육센터 — 6개 분야 PDF 추출기 (007)
=========================================================
- extract_006.py 파이프라인 재사용
- 2페이지/제안: 홀수(대안정보), 짝수(비용산출)
- 6개 파일: 건축, 구조, 기계, 전기, 조경, 토목
"""
...
OUT_DIR = BASE_DIR / "data" / "extracted_007"
OUT_DIR.mkdir(parents=True, exist_ok=True)

PROJECT_ID = "hankyong_glocal_2024"
PROJECT_NAME = "한경대학교 글로컬융합교육센터 건립공사"
SOURCE_YEAR = 2024
SOURCE_ORG = "한경대학교"

FILES = [
    ("건축", RAW_DIR / "006_한경대학교_글로컬융합교육센터_건축.pdf"),
    ("구조", RAW_DIR / "006_한경대학교_글로컬융합교육센터_구조.pdf"),
    ("기계", RAW_DIR / "006_한경대학교_글로컬융합교육센터_기계.pdf"),
    ("전기", RAW_DIR / "006_한경대학교_글로컬융합교육센터_전기.pdf"),
    ("조경", RAW_DIR / "006_한경대학교_글로컬융합교육센터_조경.pdf"),
    ("토목", RAW_DIR / "006_한경대학교_글로컬융합교육센터_토목.pdf"),
]
...
def extract_proposal_page(text):
    ...
    for i, line in enumerate(lines):
        ...
        if m and not d['field_code']:
            code = m.group(1)
            if re.match(r'(건축|구조|기계|전기|토목|조경|소방|분야)[-\s]?\d', code):
                d['field_code'] = code
        if ls in ('건축', '구조', '기계', '전기', '토목', '조경', '소방'):
            if not d['field']:
                d['field'] = ls
        if '대 안 명' in ls or '대안명' in ls:
            if i + 1 < len(lines):
    ...
    for i, line in enumerate(lines):
        if '단 점' in line.strip():
            dis = []
            for j in range(i + 1, min(i + 5, len(lines))):
                ls = lines[j].strip()
                if ls in ('시공시', '설계자', '-', ''): break
                dis.append(ls.replace('∙', '').strip())
            d['disadvantages'] = ' '.join(dis)[:200]
            break
    ...
    for i, line in enumerate(lines):
        if '검토의견' in line.strip():
            ...
            for j in range(i + 1, min(i + 5, len(lines))):
                ...
                opinion.append(ls.replace('∙', '').strip())
            d['designer_opinion'] = ' '.join(opinion)[:300]
            break
...
def insert_to_supabase(all_data):
    ...
    cur.execute("""INSERT INTO projects (project_id, project_name, file_path, total_alternatives, source_file, source_year, source_org)
        ...
        (PROJECT_ID, PROJECT_NAME, str(RAW_DIR), len(all_data), '006_한경대학교_*.pdf', SOURCE_YEAR, SOURCE_ORG))
    cur.execute("SELECT COALESCE(MAX(alt_number), 0) FROM alternatives")
    max_num = cur.fetchone()[0]
    for i, d in enumerate(all_data):
        alt_num = max_num + i + 1
        alt_id = f"hankyong_{alt_num:03d}"
        ...
        cur.execute("""INSERT INTO value_evaluations (alt_id, performance_original, performance_alternative,
            ...
            VALUES (%s,%s,%s,%s,%s,%s)""", (alt_id, 0, 0, 0, savings_rate, 0))
    pg.commit(); pg.close()
...
def run():
    print("=" * 60, flush=True)
    print("한경대학교 글로컬융합교육센터 — 6개 분야 추출", flush=True)
    print("=" * 60, flush=True)
    ...
    for field_name, pdf_path in FILES:
        ...
        # 건축/구조 파일은 첫 페이지가 '참고사항'일 수 있음 — 스킵
        for pg_idx in range(pages):
            text = doc[pg_idx].get_text()
            if '대안번호' not in text and '대 안 명' not in text:
                continue
            # 비용 페이지인지 확인 (비용산출근거)
            if '비용산출근거' in text or '비용평가' in text[:50]:
                continue

            d1 = extract_proposal_page(text)
            d2 = {}
            if pg_idx + 1 < pages:
            ...
            json_path = OUT_DIR / f"hankyong_{len(all_data)+1:03d}.json"
            with open(json_path, 'w', encoding='utf-8') as f:
                json.dump(d, f, ensure_ascii=False, indent=2)
            all_data.append(d)
            extracted += 1

        print(f"  [{field_name}] {pages}p → {extracted}건", flush=True)
        doc.close()

    print(f"\n  총 {len(all_data)}건 추출", flush=True)
    ...
    print(f"  비용절감: {cost_count}건", flush=True)
