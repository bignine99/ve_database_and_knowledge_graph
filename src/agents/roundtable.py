...
def _call_gemini(prompt: str, temperature: float = 0.5, api_key: str = "") -> str:
...
존댓말로, 회의 리더의 톤으로 작성하세요. 300자 이내."""
        ...
        msg = _call_gemini(leader_prompt, 0.4, api_key=session.api_key)
...
회의 참여자의 톤으로, 존댓말로, 250자 이내로 작성하세요."""
            ...
            msg = _call_gemini(domain_prompt, 0.5, api_key=session.api_key)
...
회의 참여자 톤으로, 300자 이내로 작성하세요."""
        ...
        idea_msg = _call_gemini(idea_prompt, 0.7, api_key=session.api_key)
...
회의 참여자 톤으로, 200자 이내로 작성하세요."""
        ...
        fast_msg = _call_gemini(fast_prompt, 0.5, api_key=session.api_key)
...
회의 참여자의 톤으로, 존댓말로, 200자 이내로 작성하세요."""
            ...
            msg = _call_gemini(feedback_prompt, 0.5, api_key=session.api_key)
...
회의 참여자 톤으로, 250자 이내로 작성하세요."""
        ...
        msg = _call_gemini(data2_prompt, 0.5, api_key=session.api_key)
...
회의 참여자 톤으로, 250자 이내로 작성하세요."""
        ...
        msg = _call_gemini(idea2_prompt, 0.6, api_key=session.api_key)
...
회의 참여자 톤으로, 200자 이내로 작성하세요."""
        ...
        msg = _call_gemini(fast2_prompt, 0.5, api_key=session.api_key)
...
회의 리더 톤으로, 존댓말로, 350자 이내로 작성하세요."""
        ...
        msg = _call_gemini(closing_prompt, 0.4, api_key=session.api_key)
...
def start_roundtable_async(project_text: str, project_name: str = "",
                           disciplines: list[str] = None, api_key: str = "") -> RoundtableSession:
